def deep_map(f, s):
    ...
    for i in range(len(s)):
        if type(s[i])==list:
            ...
            s[i]=f(s[i])
"""Cautions!!! We JUST NEED to change the value!!!"""
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        sum_left,sum_right=length(left(m))*total_mass(left_end),length(right(m))*total_mass(right_end)
        return sum_left==sum_right and balanced(left_end) and balanced(right_end)
...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    """Actually this is a question about traversing"""
    def max_path(m):
        if is_leaf(m):
            return label(m)
        else:
            if max_path(m[1])>max_path(m[2]):
                return label(m)+max_path(m[1])
            else:
                return label(m)+max_path(m[2])
    sum=max_path(t)
    print(sum)
