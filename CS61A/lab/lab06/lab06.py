...
class Account:
    ...
    def withdraw(self, amount):
        ...
        self.balance = self.balance - amount
        return self.balance
        #如果是return self.balance-amount 不会更新self.balance
...
class Server:
    ...
    """
    def __init__(self):
        self.clients = {} #初始化一个空字典

    def send(self, email):
        """Append the email to the inbox of the client it is addressed to."""
        self.clients[email.recipient_name].inbox.append(email) #这是收件人邮箱 
    def register_client(self, client):
        """Add a client to the dictionary of clients."""
        self.clients[client.name]=client #字典名[key]= value (value 是一个object 可以是class)
...
def make_change(amount, coins):
    ...
    if amount==smallest:
        ...
        s=make_change(amount-smallest,rest) #因为每一步都会移掉那个smallest 
        if s:
            return [smallest]+s
        else:
            #说明不会再使用当前smallest 那么后面则是一直清零直到有新的smallest
            return make_change(amount,rest)
            #这里可以加上smallest 但不加上也不影响 因为这意味着后续都不会使用了 故这与数量无关了

        #思路一定要清晰 换钱问题永远都是 要么用要么不用
